...
# ✅ مجلد فيديوهات الـ Hero (AI-generated يدوياً كل أسبوع)
HERO_VIDEOS_DIR = "assets/hero_videos"
...
# ✅ Step 3a (NEW): Check for a manually-generated AI hero video first
# بيدور بمرونة: أي ملف بجذر الـ repo (أو بمجلد HERO_VIDEOS_DIR لو موجود)
# يبلش بـ "hero<id>" أو "hero_<id>" وينتهي بـ .mp4 — بدون الحاجة لاسم مطابق 100%
def get_hero_video(idea):
    try:
        hotel_id = idea.get("id")
        if not hotel_id:
            print("⚠️ No 'id' column found in idea, skipping hero video check")
            return None

        hotel_id = str(hotel_id).strip()
        # الأنماط المقبولة لبداية اسم الملف
        valid_prefixes = (f"hero{hotel_id}", f"hero_{hotel_id}")

        # المجلدات يلي رح نفتش فيها: المجلد المخصص (لو موجود) + جذر الـ repo
        search_dirs = [HERO_VIDEOS_DIR, "."]
        ...
        for folder in search_dirs:
            ...
            for filename in os.listdir(folder):
                if not filename.lower().endswith(".mp4"):
                    continue
                name_no_ext = filename[:-4]  # إزالة ".mp4"
                # تأكد إنو بعد الـ prefix ما في رقم إضافي (يعني hero22 مش hero220)
                for prefix in valid_prefixes:
                    if name_no_ext == prefix or name_no_ext.startswith(prefix + "_") or name_no_ext.startswith(prefix + "-") or name_no_ext.startswith(prefix + "."):
                        full_path = os.path.join(folder, filename)
                        file_size = os.path.getsize(full_path)
                        if file_size < 10_000:
                            print(f"⚠️ Hero video candidate '{filename}' too small/corrupt ({file_size} bytes) — skipping")
                            continue
                        print(f"✅ Hero video found for id={hotel_id}: {full_path}")
                        return full_path

        print(f"ℹ️ No hero video for id={hotel_id} — will use Pexels/Pixabay/Mixkit instead")
        return None
    except Exception as e:
        print(f"❌ Hero video check failed: {e}")
        return None

# Step 3b: Get video from Pexels (✅ 3 queries for variety)
def get_pexels_video(idea):
    try:
        queries = [
            idea.get("pexels_query", idea["title"]),
            idea.get("pexels_query_2", idea["title"]),
            idea.get("pexels_query_3", idea["title"]),
        ]
...
# Step 3c: Get video from Pixabay as second source
...
# Step 3d: Get video from Mixkit as third source
def get_mixkit_video(idea):
    try:
        mixkit_luxury = [
            "https://assets.mixkit.co/videos/4075/4075-720.mp4",
            "https://assets.mixkit.co/videos/3120/3120-720.mp4",
            "https://assets.mixkit.co/videos/2366/2366-720.mp4",
            "https://assets.mixkit.co/videos/1393/1393-720.mp4",
            "https://assets.mixkit.co/videos/5765/5765-720.mp4",
            "https://assets.mixkit.co/videos/4953/4953-720.mp4",
            "https://assets.mixkit.co/videos/3123/3123-720.mp4",
            "https://assets.mixkit.co/videos/2879/2879-720.mp4",
            "https://assets.mixkit.co/videos/5152/5152-720.mp4",
            "https://assets.mixkit.co/videos/4824/4824-720.mp4",
            "https://assets.mixkit.co/videos/3947/3947-720.mp4",
            "https://assets.mixkit.co/videos/2993/2993-720.mp4",
            "https://assets.mixkit.co/videos/1598/1598-720.mp4",
            "https://assets.mixkit.co/videos/5432/5432-720.mp4",
            "https://assets.mixkit.co/videos/4201/4201-720.mp4",
            "https://assets.mixkit.co/videos/3567/3567-720.mp4",
        ]
        video_url = random.choice(mixkit_luxury)
        print(f"✅ Mixkit video: {video_url}")
        return video_url
    except Exception as e:
        print(f"❌ Mixkit failed: {e}")
        return None

# Step 3e: Generate video with FAL.ai as last fallback
...
# ✅ Step 4: Send to Telegram (now handles BOTH local hero videos and remote URLs)
def send_to_telegram(video_source, caption, is_local_file=False):
    try:
        if is_local_file:
            print(f"📂 Using local hero video file: {video_source}")
            with open(video_source, "rb") as f:
                video_bytes = f.read()
        else:
            print("⬇️ Downloading video...")
            video_data = requests.get(video_source, timeout=60)
            if video_data.status_code != 200:
                print("❌ Failed to download video")
                return False
            video_bytes = video_data.content
...
# ✅ Try Hero (local AI video) → Pexels → Pixabay → Mixkit → FAL.ai
video_source = get_hero_video(idea)
is_local = video_source is not None

if not video_source:
    video_source = get_pexels_video(idea)

if not video_source:
    print("⚠️ Pexels failed — trying Pixabay...")
    video_source = get_pixabay_video(idea)

if not video_source:
    print("⚠️ Pixabay failed — trying Mixkit...")
    video_source = get_mixkit_video(idea)

if not video_source:
    print("⚠️ Mixkit failed — trying FAL.ai...")
    video_source = generate_fal_video(idea)

if not video_source:
    print("❌ No video available from any source. Exiting.")
    exit(1)

send_to_telegram(video_source, caption, is_local_file=is_local)
