...
def get_hero_video(idea):
    try:
        ...
        expected_path = os.path.join(HERO_VIDEOS_DIR, f"hero_{hotel_id}.mp4")
        ...
        if file_size < 10_000:
            print(f"⚠️ Hero video for id={hotel_id} looks too small/corrupt ({file_size} bytes) — skipping")
            return None

        print(f"✅ Hero video found for id={hotel_id}: {expected_path}")
        # محلي (مش رابط)، فبنرجع المسار مباشرة
